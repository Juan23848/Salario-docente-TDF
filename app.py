...
if st.button("Calcular sueldo"):
    ...
    # Calcular FOID y Ayuda Material sumando las equivalencias
    horas_equivalentes = sum(item["puntaje"] * item["cantidad"] for item in cargos)
    foid = calcular_foid(horas_equivalentes, 0)
    ayuda = calcular_ayuda_material(horas_equivalentes, 0)
    ...
    total_horas = sum(item["puntaje"] * item["cantidad"] for item in cargos)
    if total_horas > 54:
